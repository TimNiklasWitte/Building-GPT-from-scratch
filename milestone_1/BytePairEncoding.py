...
class BytePairEncoding:
    ...
    def train(self, corpus):
        ...
        map_word_cnt = {}       # count of each word
        map_word_tokens = {}    # tokens of each word
        ...
        for line in lines:
            ...
            for word in line:
                word = self.normalize_word(word) # normalize word to lower case
                word = word + ' '                # add dummy space at the end to avoid empty tokenization

                try:                        # count word
                    map_word_cnt[word] += 1 # if word already exists, increment count
                except KeyError:
                    map_word_cnt[word] = 1 # first time, set count to 1
